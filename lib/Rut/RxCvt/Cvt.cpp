...
namespace Rut::RxCvt
{
	...
	size_t WStrToStr(const wchar_t* wpStr, size_t wsStrLen, char* cpStr, size_t msStrbytes, size_t nCodePage, bool* isError, const char* cpDefault)
	{
		BOOL not_all_cvt = TRUE;
		size_t size = WideCharToMultiByte((UINT)nCodePage, 0, wpStr, (int)wsStrLen, cpStr, (int)msStrbytes, cpDefault, &not_all_cvt);
		if (isError)
		{
			*isError = not_all_cvt;
		}
...
namespace Rut::RxCvt
{
	...
	size_t StrToWStr(const char* cpStr, size_t msStrbytes, wchar_t* wpStr, size_t wsStrCCH, size_t nCodePage)
	{
		return MbcsStrToUnicodeStr(cpStr, wpStr, nCodePage);
	}

	// IF cpStr == nullptr return Multibyte String bytes
	size_t WStrToStr(const wchar_t* wpStr, size_t wsStrCCH, char* cpStr, size_t msStrbytes, size_t nCodePage, bool* isError, const char* cpDefault)
	{
		if (nCodePage == 65001)
		{
